...
export async function getResourcePlanSummary(options: {
  ...
  for (const task of tasks) {
    ...
    const visibleAssignees = task.assignees.filter(({ userId }) => employeeIdSet.has(userId));
    ...
    if (task.estimatedMinutes > 0 && actualMinutes > task.estimatedMinutes) overEstimateTasks += 1;
  }

  const holidayDates = new Set(holidays.map(({ date }) => isoDate(date)));
  const leavesByEmployee = new Map<string, typeof leaves>();
  for (const leave of leaves) {
    const rows = leavesByEmployee.get(leave.userId) ?? [];
    rows.push(leave);
    leavesByEmployee.set(leave.userId, rows);
  }
  const lastDay = new Date(weekEnd);
  lastDay.setUTCDate(lastDay.getUTCDate() - 1);
  ...
  for (const employee of employees) {
    ...
    const leaveMinutes = (leavesByEmployee.get(employee.id) ?? []).reduce((total, leave) => total + calculateWorkingLeaveMinutes({
    ...
    if (loadPercent > 110) overloadedEmployees += 1;
