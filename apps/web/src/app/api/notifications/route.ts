...
export async function GET(request: NextRequest) {
  ...
  const [unreadCount, latest] = await Promise.all([
    ...
    prisma.notification.findFirst({
      ...
      select: { id: true, titleEn: true, titleAr: true, bodyEn: true, bodyAr: true, href: true, createdAt: true, updatedAt: true },
    }),
  ]);
  return Response.json({
    ...
    latest: latest ? {
      ...
      version: `${latest.id}:${latest.updatedAt.toISOString()}`,
    } : null,
