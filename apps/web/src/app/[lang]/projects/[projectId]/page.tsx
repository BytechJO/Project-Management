    ...
    >
      ...
      {activeTimer?.startedAt ? (
        <section className={projectStyles.timerBanner}>
          ...
          <form action={stopTaskTimer}>
            ...
            <PendingSubmitButton className={projectStyles.stopButton} pendingLabel={isArabic ? "جارٍ الحفظ..." : "Saving..."}>{isArabic ? "إيقاف وحفظ" : "Stop & save"}</PendingSubmitButton>
          </form>
        </section>
      ) : null}
      ...
      <section className={`${styles.panel} ${projectStyles.projectFilesPanel}`}>
        ...
        {oneDriveConnection ? <form action={uploadProjectAttachment} className={projectStyles.fileUploadForm}>
          ...
          <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الرفع..." : "Uploading..."}>{isArabic ? "رفع إلى OneDrive" : "Upload to OneDrive"}</PendingSubmitButton>
                  ...
                  return <tr key={task.id}>
                    ...
                    <td><div className={projectStyles.taskActions}>{canManageTasks || assignedToCurrentUser ? (
                      <form action={updateTaskWorkflowStatus} className={projectStyles.statusForm}>
                        ...
                        <PendingSubmitButton className={projectStyles.smallButton} name="status" pendingLabel={isArabic ? "جارٍ التحديث..." : "Updating..."} value={nextTaskStatus[task.status]}>{taskActionLabel(task.status, isArabic)}</PendingSubmitButton>
                      </form>
                    ) : <span className={styles.badge}>{task.status.replaceAll("_", " ")}</span>}
                    {timerIsForTask && activeTimer ? (
                      <form action={stopTaskTimer}>
                        ...
                        <PendingSubmitButton className={projectStyles.stopButton} pendingLabel={isArabic ? "جارٍ الحفظ..." : "Saving..."}>{isArabic ? "إيقاف" : "Stop timer"}</PendingSubmitButton>
                      </form>
                    ) : timerCanStart ? (
                      <form action={startTaskTimer}>
                        <input name="locale" type="hidden" value={lang} /><input name="projectId" type="hidden" value={project.id} /><input name="taskId" type="hidden" value={task.id} />
                        <PendingSubmitButton className={projectStyles.timerButton} pendingLabel={isArabic ? "جارٍ البدء..." : "Starting..."}>{isArabic ? "بدء التايمر" : "Start timer"}</PendingSubmitButton>
                      </form>
          ...
          {canManageTasks ? (
            <section className={styles.panel}>
              ...
              <form action={createTask} className={projectStyles.form}>
                ...
                <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الإضافة..." : "Adding..."}>{isArabic ? "إضافة التاسك" : "Add task"}</PendingSubmitButton>
              </form>
            </section>
          ) : null}
        </div>
          ...
          {canManageProject ? (
            <section className={styles.panel}>
              ...
              <form action={assignProjectMember} className={projectStyles.form}>
                ...
                <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الإضافة..." : "Adding..."}>{isArabic ? "إضافة للفريق" : "Add to team"}</PendingSubmitButton>
              </form>
            </section>
          ) : null}
