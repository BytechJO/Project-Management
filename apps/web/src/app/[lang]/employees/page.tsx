...
export default async function EmployeesPage({
  ...
  const [employeeRows, departments, roles] = await Promise.all([
    prisma.user.findMany({
      ...
      select: {
        ...
        costRates: {
          select: { currency: true, hourlyCost: true, monthlyAllowances: true, monthlyBenefits: true, monthlySalary: true },
    ...
    prisma.department.findMany({ where: { organizationId: user.organizationId!, isActive: true }, select: { id: true, name: true }, orderBy: { name: "asc" } }),
    prisma.role.findMany({ where: { organizationId: user.organizationId!, scope: "ORGANIZATION" }, select: { id: true, name: true }, orderBy: { name: "asc" } }),
  ]);
  const hasNextPage = employeeRows.length > DEFAULT_PAGE_SIZE;
  const employees = employeeRows.slice(0, DEFAULT_PAGE_SIZE);
    ...
    <AppShell activeSection="employees" dictionary={dictionary} locale={lang}>
      ...
      <div className={managementStyles.layout}>
        <section className={`${styles.panel} ${styles.tableWrap}`}>
          ...
          <Pagination basePath={`/${lang}/employees`} currentPage={currentPage} hasNextPage={hasNextPage} isArabic={isArabic} />
        </section>
