      ...
      <div className={styles.headingRow}>
        ...
        {canApprove && approvalQueue.length > 0 ? (
          <form action={approveVisibleTimesheets}>
            ...
            <PendingSubmitButton className={styles.secondaryButton} pendingLabel={isArabic ? "جارٍ الاعتماد..." : "Approving..."}>{page.approveVisible}</PendingSubmitButton>
          </form>
        ) : null}
      </div>
      ...
      {activeTimer?.startedAt ? (
        <section className={moduleStyles.activeTimer}>
          ...
          <form action={stopTaskTimer}>
            ...
            <PendingSubmitButton className={moduleStyles.stopTimerButton} pendingLabel={isArabic ? "جارٍ الحفظ..." : "Saving..."}>{isArabic ? "إيقاف وحفظ" : "Stop & save"}</PendingSubmitButton>
          </form>
        </section>
      ) : null}
        ...
        <section className={styles.panel}>
          ...
          {myTimesheet && canEditMyTimesheet && myTimesheet.entries.length > 0 && !activeTimer ? (
            <form action={submitTimesheet} className={moduleStyles.submitRow}>
              ...
              <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الإرسال..." : "Submitting..."}>{isArabic ? "إرسال للموافقة" : "Submit for approval"}</PendingSubmitButton>
            </form>
          ...
          {canEditMyTimesheet ? (
            tasks.length ? (
              <form action={createManualTimeEntry} className={managementStyles.form}>
                ...
                <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الإضافة..." : "Adding..."}>{isArabic ? "إضافة الساعات" : "Add hours"}</PendingSubmitButton>
              </form>
                  ...
                  <article className={moduleStyles.approvalCard} key={timesheet.id}>
                    ...
                    <div className={moduleStyles.reviewActions}>
                      <form action={approveTimesheet}>
                        ...
                        <PendingSubmitButton className={styles.button} pendingLabel={isArabic ? "جارٍ الاعتماد..." : "Approving..."}>{page.approve}</PendingSubmitButton>
                      </form>
                      <form action={returnTimesheet} className={moduleStyles.returnForm}>
                        ...
                        <PendingSubmitButton className={styles.secondaryButton} pendingLabel={isArabic ? "جارٍ الإرجاع..." : "Returning..."}>{isArabic ? "رفض / إرجاع" : "Return / reject"}</PendingSubmitButton>
                      </form>
                    </div>
                  </article>
                );
