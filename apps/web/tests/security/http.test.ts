...
describe("operational health endpoint", () => {
  test("returns only a safe readiness result and is never cached", async () => {
    ...
    assert.equal(body.service, "bytech-project-management");
    assert.equal(body.status, response.status === 200 ? "ok" : "unavailable");
    assert.equal(body.checks.database, response.status === 200 ? "ok" : "unavailable");
    assert.equal(Number.isFinite(body.responseTimeMs), true);
