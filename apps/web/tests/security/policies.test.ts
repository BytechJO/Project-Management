...
import { sanitizeLogValue } from "../../src/lib/log-safety";
...
describe("safe public output", () => {
  test("removes credentials and log-injection characters from operational logs", () => {
    const sanitized = sanitizeLogValue("Database failed\r\nfor postgresql://admin:top-secret@db:5432/app password=second-secret");
    assert.equal(sanitized.includes("top-secret"), false);
    assert.equal(sanitized.includes("second-secret"), false);
