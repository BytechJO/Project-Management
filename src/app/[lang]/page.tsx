...
export default async function DashboardPage({ params, searchParams }: PageProps<"/[lang]">) {
  ...
  const trackedMinutes = resourcePlan?.summary.totalActualMinutes ?? 0;
  const capacityMinutes = resourcePlan?.summary.totalCapacityMinutes ?? user.weeklyCapacityMinutes;
  const plannedMinutes = resourcePlan?.summary.totalPlannedMinutes ?? 0;
  const plannedLoad = resourcePlan?.summary.plannedLoadPercent ?? 0;
  const openProjects = projects.filter(({ status }) => ["ACTIVE", "PLANNED", "ON_HOLD"].includes(status));
  const dueThisMonth = openProjects.filter(({ targetDate }) => targetDate && targetDate >= monthStart && targetDate < monthEnd).length;
  ...
  const portfolioMargin = financialPortfolio?.totals.forecastMargin ?? 0;
  ...
  const featuredProjects = openProjects.slice(0, 5);

  const metrics = canViewTeamProjects
    ? [
        { label: isArabic ? "المشاريع المفتوحة" : "Open projects", value: String(openProjects.length), note: isArabic ? `${dueThisMonth} مستحق هذا الشهر` : `${dueThisMonth} due this month` },
        ...
        canViewFinancials
          ? { label: isArabic ? "هامش المشاريع المتوقع" : "Forecast portfolio margin", value: `${portfolioMargin.toFixed(1)}%`, note: isArabic ? "حسب تكلفة الإتمام المتوقعة" : "From estimate at completion" }
    ...
    : [
        { label: isArabic ? "مشاريعي المفتوحة" : "My open projects", value: String(openProjects.length), note: isArabic ? "المشاريع المسندة إليّ" : "Projects assigned to me" },
        ...
        { label: isArabic ? "ساعاتي هذا الأسبوع" : "My hours this week", value: (ownMinutes / 60).toFixed(1), note: ownTimesheet?.status.replaceAll("_", " ") ?? (isArabic ? "لم يبدأ السجل" : "Timesheet not started"), tone: "accent" },
      ...
      <div className={styles.dashboardGrid}>
        <section className={sectionStyles.panel}>
          <div className={sectionStyles.panelHeader}>
            ...
            <span className={sectionStyles.badge}>{openProjects.length} {isArabic ? "مفتوح" : "open"}</span>
          </div>
        ...
        <section className={sectionStyles.panel}>
          ...
          <div className={styles.attentionList}>
            <Link href={`/${lang}/resource-planning`}>
              ...
              <span className={resourcePlan?.summary.overloadedEmployees || resourcePlan?.summary.overEstimateTasks ? sectionStyles.warningBadge : sectionStyles.badge}>{(resourcePlan?.summary.overloadedEmployees ?? 0) + (resourcePlan?.summary.overEstimateTasks ?? 0)}</span>
            </Link>
            <Link href={`/${lang}/timesheets`}>
              <div><strong>{isArabic ? "سجلات الوقت" : "Timesheets"}</strong><span>{canApproveTimesheets ? (isArabic ? "بانتظار الموافقة" : "Waiting for approval") : (ownTimesheet?.status.replaceAll("_", " ") ?? (isArabic ? "لم يبدأ" : "Not started"))}</span></div>
              <span className={pendingTimesheets || ownTimesheet?.status === "REJECTED" ? sectionStyles.warningBadge : sectionStyles.badge}>{canApproveTimesheets ? pendingTimesheets : `${(ownMinutes / 60).toFixed(1)} h`}</span>
            </Link>
            ...
            <Link href={permissions.has("subscriptions.manage") ? `/${lang}/subscriptions` : canViewSubscriptions ? `/${lang}/financials` : `/${lang}/projects`}>
